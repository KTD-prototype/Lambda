...
void PostureAngle() {
  ...
  ACCroll = 0.05 * ACCroll + 0.95 * atan(data[1] / data[2]) * 180 / M_PI;
  ACCpitch = 0.05 * ACCpitch + 0.95 * atan(data[0] / sqrt(pow(data[1], 2) + pow(data[2], 2))) * 180 / M_PI;
  ...
  //roll = roll + data[3]  / SamplingRate;
  //pitch = pitch + (-1 * data[4]) / SamplingRate;
  if (abs(data[5]) < 0.1) {
    yaw = yaw;
  }
