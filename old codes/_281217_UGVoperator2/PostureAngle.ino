...
float data[6] = {0, 0, 0, 0, 0, 0}; //加速度3軸＋角速度3軸
float Calib[6] = {0, 0, 0, 0, 0, 0}; //同上。キャリブレーション用。
float ACCroll = 0, ACCpitch = 0, ACCyaw = 0; //加速度から計算した姿勢。
...
void PostureAngle() {
  ...
  ACCroll = 0.75 * ACCroll + 0.25 * atan2(data[1],  data[2]) * 180 / M_PI;
  ACCpitch = 0.75 * ACCpitch + 0.25 * atan2(data[0], sqrt(pow(data[1], 2) + pow(data[2], 2))) * 180 / M_PI;
