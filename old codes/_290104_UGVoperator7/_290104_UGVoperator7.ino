...
byte str[20]; //ラズパイからの制御信号を格納する関数
byte discard;
unsigned long t;
...
void setup() {
  //シリアル通信開始。
  JOY_receive.begin(9600); //無線通信モジュールとの通信ポート
  Serial.begin(115200); //ラズパイとの通信ポート
  Wire.begin(); //I2C通信開始
...
void loop() {
  ...
  if (JOY_receive.available() > 6) {  //受信バッファにデータが6バイトより多くある時（ヘッダバイト＋ジョイスティック信号６バイト
    if (JOY_receive.read() == 'H') {
      ...
      if (mode == 0 || mode == 2) {
        ...
        mode_A(); //ラズパイからの指令値に基づきモータを駆動する関数
      }
    }
    ...
    //（各モードの最初に実行）JOY_receive.write(mode);//現在の動作モードをコントローラにフィードバックする
    ...
    wait++;
    if (wait == 2000) {//2000ループ（1ループ2～3msとすると最大6秒くらい）経っても6バイト貯まらなかったら、通信断と判定しmodeをSTOPとする。
      mode = 0;
      while (JOY_receive.available() > 0) {
        discard = JOY_receive.read(); //受信バッファが空になるまでデータを読み出して、リセット。
      }
  ...
  if (flag == 1) {//20msごとにタイマー割込みによりフラグが立つ。フラグが立っていたら、IMU情報を取得し、姿勢角を計算。
