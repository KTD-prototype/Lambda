...
float P = 30, D = 0.8, I = 5;  //PIDの係数
float vel = 0, prevel = 0; //モータ目標速度、前回目標速度
float target_pitch = 2; //ピッチ角ターゲット=安定になる角度。とりあえず0度にしておく
float integral_pitch_error = 0; //ピッチ角誤差積算
...
void loop() {
  while (abs(data[0]) < 5 && abs(data[1]) < 5 && abs(data[2]) < 5) {
    //1000ループ目で一度誤差積算（I制御用）をリセットする。
    //pitch角変数の初期値と、ロボット電源投入時のピッチ角が合わず、余計に誤差積算してしまうため
    if (firstloop == 1000) {
      //安全のため、すべてのモータ出力をゼロに
      //analogWrite(5, 0);
      //analogWrite(6, 0);
    ...
    //ループカウントをカウントアップ
    firstloop++;
    if (firstloop > 2000) {//オーバーフロしないよう2000でうちどめ
      firstloop = 2000;
    }
  }
  analogWrite(5, 0);
